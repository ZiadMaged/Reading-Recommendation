...
import { ResponseBookProgressDto, ResponseRecommendationBookDto, ResponseUserBookDto, UpdateBookProgressDto, UsersBooksPaginationDto } from '../utils/users-books.dto';
...
export class UsersBooksService {
  ...
  async getRecommendationBooks(): Promise<ResponseRecommendationBookDto[]> {
    const { rows }: UsersBooksPaginationDto = await this.usersBooksRepository.getAll({
      attributes: [
        'bookId',
    ...
    const response: ResponseRecommendationBookDto[] = rows.map(b => ({
      book_id: b.bookId,
  ...
  async addBookProgress(
    ...
    this.validateIntervals(payload.startPage, payload.endPage);

    const userBook: ResponseUserBookDto = await this.usersBooksRepository.get(null, {
      where: {
    ...
    if (payload.startPage >= userBook.startPage)
      delete payload.startPage;

    if (payload.endPage <= userBook.endPage)
      delete payload.endPage;
  ...
  private validateIntervals(startPage: number, endPage: number) {
